...
(function () {
  ...
  function AuthGate({ children }) {
    ...
    if (session) return <React.Fragment>
      ...
      {subscriptionPlan && <button className="ss-manage" onClick={paidMember ? manageMembership : () => setError("This is a 24-hour promotional trial, so there is no paid membership to cancel. A paid subscription can be managed or canceled here at any time.")} disabled={busy}>{paidMember ? "Manage or cancel membership" : "Manage or cancel membership (trial)"}</button>}
