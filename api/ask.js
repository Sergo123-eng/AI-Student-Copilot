...
For an academic explanation, answer the student's question first. Use a short, concrete analogy when their plan allows it. Do not replace the explanation with study-method advice, a referral, or a request to ask an instructor unless the question requires a school-specific policy you cannot verify.`;
  ...
  try {
    ...
    // General, source-backed answers from the shared cache do not spend an AI
    // allowance. A student can benefit from answers already paid for by the
    // service while personal prompts remain private and never enter the cache.
  ...
  try {
    const usage = await consumePlanUsage(access.email, access.usagePlan || access.plan, latestQuestion);
