...
function significantTerms(question) {
  ...
  return new Set(normalizedQuestion(question).split(" ").filter(word => word.length > 2 && !ignored.has(word)).map(stem));
}

function stem(word) {
  return String(word || "")
    .replace(/(ations|ation|ments|ment|ingly|ing|edly|ed|ies|es|s)$/i, "")
    .replace(/[^a-z0-9]/gi, "");
}

function questionIntent(question) {
  const text = normalizedQuestion(question);
  if (/\b(practice|quiz|flashcard|test me)\b/.test(text)) return "practice";
  if (/\b(source|reference|journal|paper|research)\b/.test(text)) return "sources";
  if (/\b(analogy|like im|simple)\b/.test(text)) return "simple";
  if (/\b(explain|define|what is|how does|teach me)\b/.test(text)) return "explain";
  return "general";
}

function lexicalSimilarity(a, b) {
  const left = significantTerms(a), right = significantTerms(b);
  if (!left.size || !right.size || questionIntent(a) !== questionIntent(b)) return 0;
  let overlap = 0;
  for (const word of left) if (right.has(word)) overlap += 1;
  // Short academic prompts often have one distinctive topic word, such as
  // "photosynthesis".  Treat that as a close match only when the request
  // shape is also the same; this saves a repeat model call without mixing a
  // practice request, reference search, or personal guidance into an answer.
  if (overlap && (left.size < 3 || right.size < 3)) {
    const hasDistinctiveTopic = [...left].some(word => right.has(word) && word.length >= 7);
    return hasDistinctiveTopic ? 0.85 : 0;
  }
...
const USAGE_LIMITS = {
  ...
  // Promo trials have a short 24-hour life and materially lower ceilings than
  // their matching paid plans. These numbers stay server-side by design.
  day_promo: { guidance: 5, reference: 1, practice: 0, analogy: 0 },
  ...
  plus_promo: { guidance: 25, reference: 2, practice: 1, analogy: 0 },
  pro: { guidance: 150, reference: 7, practice: 10, analogy: 30 },
  pro_promo: { guidance: 50, reference: 3, practice: 3, analogy: 8 },
  super: { guidance: 200, reference: 20, practice: 20, analogy: 40 },
  super_promo: { guidance: 75, reference: 5, practice: 5, analogy: 12 },
