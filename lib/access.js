...
export function issueAccess(res, data) {
  const payload = b64(JSON.stringify({ ...data, exp: Date.now() + (data.plan === "day" ? 86400000 : 2592000000) }));
  const token = `${payload}.${sign(payload)}`;
  res.setHeader("Set-Cookie", `${COOKIE_NAME}=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${data.plan === "day" ? 86400 : 2592000}`);
}
